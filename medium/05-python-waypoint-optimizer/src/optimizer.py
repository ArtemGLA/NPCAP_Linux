...
def calculate_distance_matrix(waypoints: list[dict]) -> np.ndarray:
    ...
    TODO: Реализуйте расчёт матрицы расстояний
    
    Для каждой пары точек вычислить 3D расстояние:
    - horizontal = haversine_distance(lat1, lon1, lat2, lon2)
    - vertical = abs(alt1 - alt2)
    - total = sqrt(horizontal^2 + vertical^2)
    ...
    # TODO: Заполните матрицу расстояниями
    # 
    # for i in range(n):
    #     for j in range(n):
    #         if i != j:
